...
class BidirectionalAttentionMixnet(nn.Module):
    def __init__(
        ...
        if self.use_distribution:
            for _ in range(n_linear_layers-1):
                ...
                self.linear_layers.append(nn.LeakyReLU().to(device))
                # self.linear_layers.append(nn.LayerNorm(z_dim).to(device))
            self.output = nn.Linear(hidden_dim, self.num_atoms).to(device)
            self.softmax = nn.Softmax(dim=-1)
            # self.linear = Linear_block(hidden_dim, n_linear_layers, device)
            # self.linear_ensemble = [Linear_block(hidden_dim, n_linear_layers) for _ in range(self.ensemble_size)]
        else:
            for _ in range(n_linear_layers-1):
                self.linear_layers.append(nn.Linear(hidden_dim, hidden_dim).to(device))
                # self.linear_layers.append(nn.LeakyReLU().to(device))
    ...
    def forward(self, forward_rep, backward_rep):
        ...
        for layer in self.linear_layers:
            ...
            if self.use_film_cond and self.file_cnt == 0:
                gamma, beta = self.film_generator(backward_rep).chunk(2, dim=-1)
                combined_output = combined_output * gamma + beta
                self.file_cnt = 1
            linear_cnt += 1
        if self.use_distribution:
            # u_set, std_set = [], []
...
class MixNetRepresentation(torch.nn.Module):
    def __init__(
        ...
        use_OFE: bool,
        phi_o_hidden_layers: int = 2,
        phi_ao_hidden_layers: int = 2,
        ensemble_size: int = 0,
        num_atoms: int = 51,
        ...
        self.forward_representation = ORERepresentation(
            ...
            device=device,
        ) if use_OFE else ForwardRepresentation(
            observation_length=observation_length,
            action_length=action_length,
            preprocessor_hidden_dimension=preprocessor_hidden_dimension,
            preprocessor_feature_space_dimension=preprocessor_feature_space_dimension,
        ...
        self.forward_representation_target = ORERepresentation(
            observation_length=observation_length,
            action_length=action_length,
            z_dimension=z_dimension,
            preprocessor_hidden_dimension=preprocessor_hidden_dimension,
        ...
        self.forward_mu, self.forward_mu_target = (
            ForwardModel(
                preprocessor_feature_space_dimension=observation_length,
