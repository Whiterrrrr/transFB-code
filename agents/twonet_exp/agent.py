...
from agents.twonet_exp.module import deepNetApproximator

class EXP_2Net(AbstractAgent):
    ...
    def __init__(
        ...
        self.approximator.M1_target.load_state_dict(
            self.approximator.M1.state_dict()
        )
        self.approximator.M2_target.load_state_dict(
            self.approximator.M2.state_dict()
        )
        self.approximator.Q1_target.load_state_dict(
            self.approximator.Q1.state_dict()
        )
        self.approximator.Q2_target.load_state_dict(
            self.approximator.Q2.state_dict()
        )
        ...
        self.critic_optimizer = torch.optim.Adam(
            [
                {"params": self.approximator.M1.parameters()},
                {"params": self.approximator.M2.parameters()},
                {"params": self.approximator.Q1.parameters()},
                {"params": self.approximator.Q2.parameters()},
                {
    ...
    def update(self, batch: Batch, batch_rand: Batch, step: int) -> Dict[str, float]:
        ...
        self.soft_update_params(
            network=self.approximator.M1,
            target_network=self.approximator.M1_target,
            tau=self._tau,
        )
        self.soft_update_params(
            network=self.approximator.M2,
            target_network=self.approximator.M2_target,
            tau=self._tau,
        )
        self.soft_update_params(
            network=self.approximator.Q1,
            target_network=self.approximator.Q1_target,
            tau=self._tau,
        )
        self.soft_update_params(
            network=self.approximator.Q2,
            target_network=self.approximator.Q2_target,
            tau=self._tau,
        )
        self.soft_update_params(
    ...
    def _update_operate_inner(
        ...
        with torch.no_grad():
            ...
            target_M1 = self.approximator.M1_target(observation=next_observations, z=zs, action=next_actions, B=target_B)
            target_M2 = self.approximator.M2_target(observation=next_observations, z=zs, action=next_actions, B=target_B)
        ...
        M1_next = self.approximator.M1(observation=observations, z=zs, action=actions, B=B_next)
        M2_next = self.approximator.M2(observation=observations, z=zs, action=actions, B=B_next)
        M1_rand = self.approximator.M1(observation=observations, z=zs, action=actions, B=B_rand)
        M2_rand = self.approximator.M2(observation=observations, z=zs, action=actions, B=B_rand)
        ...
        with torch.no_grad():
            next_Q1 = self.approximator.Q1_target(observation=next_observations, z=zs, action=next_actions).squeeze() 
            next_Q2 = self.approximator.Q2_target(observation=next_observations, z=zs, action=next_actions).squeeze()
        ...
        Q1 = self.approximator.Q1(observation=observations, z=zs, action=actions).squeeze()
        Q2 = self.approximator.Q2(observation=observations, z=zs, action=actions).squeeze()
        Q = torch.min(Q1, Q2)
    ...
    def update_actor(
        ...
        Q1 = self.approximator.Q1(observation=observation, action=action, z=z).squeeze()
        Q2 = self.approximator.Q2(observation=observation, action=action, z=z).squeeze()
        Q = torch.min(Q1, Q2)
    ...
    def predict_q(
        ...
        Q1 = self.approximator.Q1(observation, action, z)
        Q2 = self.approximator.Q2(observation, action, z)
        Q = torch.min(Q1, Q2)
    ...
    def _value_conservative_penalty(
        ...
        # convert to Qs
        cql_cat_Q1 = self.approximator.Q1(observation=cat_obs, action=cat_actions, z=cat_zs).reshape(
            self.total_action_samples, self.batch_size, -1
        )
        cql_cat_Q2 = self.approximator.Q2(observation=cat_obs, action=cat_actions, z=cat_zs).reshape(
            self.total_action_samples, self.batch_size, -1
        )
